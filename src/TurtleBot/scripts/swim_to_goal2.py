    ...
    while not rospy.is_shutdown():
        ...
        if not is_arrived:
            ...
            while not is_facing_target and not rospy.is_shutdown():
                # turn to face target
                is_facing_target = check_body_angle(
                    pos_node.data, target_point)
                ...
                if iterations % 10 == 0:
                    rospy.loginfo('err_pos = %.2f; err_ang = %.2f' %
                                  (err_pos, err_ang))
                    rospy.loginfo('lin_vel = %.2f; ang_vel = %.2f' %
                                  (lin_vel, ang_vel))
...
def request_number(var_name, is_bounds=True, bounds=(1, 9)):
    ...
    if is_bounds:
        while not is_valid:
            val = click.prompt(
                'Enter %s coordinate between %.2f-%.2f or q to exit' % (var_name, min_in, max_in))
            try:
                val = float(val)
            except:
                if val == 'q':
            ...
            if val >= min_in and val <= max_in:
                ...
                rospy.loginfo('Enter number between %.2f-%.2f' %
                              (min_in, max_in))
    else:
        while not is_valid:
