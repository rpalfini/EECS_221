...
def main_swim():
    ...
    # prev_angle = pos_node.prev_data.theta
    ang_rotate_total = 0
    while not rospy.is_shutdown():
        cur_angle = pos_node.data.theta
        ang_rotate_iter = cur_angle - prev_angle
        if abs(ang_rotate_iter) > 1:
            ang_rotate_iter = cur_angle + prev_angle
        ang_rotate_total += ang_rotate_iter
        # if iter_count % 1 == 0:
        #     rospy.loginfo('cur_angle = %.3f; prev_angle = %.3f; ang_rotate_iter = %.3f; ang_rotate_total = %.3f' % (cur_angle,prev_angle,ang_rotate_iter,ang_rotate_total))
